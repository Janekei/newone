 ...
 <Dialog v-model="dialogVisible" :title="dialogTitle">
  <el-form ref="formRef" v-loading="formLoading" :model="formData" :rules="formRules" label-width="100px">
   ...
   <el-form-item v-if="formData.type !== 1" label="权限标识" prop="permission">
    <template #label>
     <Tooltip message="Controller 方法上的权限字符，如：@PreAuthorize(`@ss.hasPermission('system:user:list')`)" titel="权限标识" />
    </template>
   ...
   <!-- <el-form-item label="新标签页">
    <el-radio-group v-model="formData.newPage">
...
const formRules = reactive({
 ...
 newPage: [{ required: true, message: '不能为空', trigger: 'blur' }]
})
...
const open = async (type: string, id?: number, parentId?: number) => {
 ...
 if (id) {
  ...
  try {
   ...
   console.log(formData.value, 145);
...
const submitForm = async () => {
 ...
 try {
  ...
  console.log(formData.value, 171);
